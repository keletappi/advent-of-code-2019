...
fun parse(wire: String): List<Coord> {
    ...
    wire.split(",")
            .forEach {
                when (it.get(0)) {
                    'R' -> {
                        0.until(it.substring(1).toInt()).forEach {
                            loc = Coord(loc.x + 1, loc.y, loc.n + 1)
                            result.add(loc)
                        }
                    }
                    'L' -> {
                    ...
                    'U' -> {
                        0.until(it.substring(1).toInt()).forEach {
                            loc = Coord(loc.x, loc.y + 1, loc.n + 1)
                            result.add(loc)
                        }
                    }
                    'D' -> {
...
private fun solveMinimumManhattan(firstRoute: List<Coord>, secondRoute: List<Coord>): Coord {
    return firstRoute
            .filter { secondRoute.contains(it) }
            .reduce { acc: Coord, c: Coord -> if (acc.manhattan < c.manhattan) acc else c }
}

private fun solveMinimumSteps(firstRoute: List<Coord>, secondRoute: List<Coord>): Int {
    return firstRoute.filter { secondRoute.contains(it) }
            .map { ittt -> ittt.n + secondRoute.first { itt -> ittt == itt }.n }
            .min()!!
}
...
data class Coord(val x: Int, val y: Int, val n: Int) {
    ...
    override fun toString(): String {
        return "Coord(x=$x, y=$y, n=$n -> man=$manhattan)"
    }
}